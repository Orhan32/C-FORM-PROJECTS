...
(function ( $ ) {
	...
	$.toast = function(options){
		...
		$.each(options.buttons,function(b, button){
			var content = "<button class='btn btn-default'>" + button.label + "</button>"
			if (b === 2) {
				content = "<br/>" + content;
			}
			var hb = $(content);
			if (button.label === chrome.i18n.getMessage("donate")) {
				hb = $("<button class='btn btn-primary'>" + button.label + "</button>");
			}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    (function ($) {
                        ...
                        $.toast({
                            ...
                            buttons: [
                                ...
                                },
                                {
                                    ...
                                    callback: function () {
                                        window.open(args[0] || null, args[1] || null, args[2] || null);
                                        addScript({ textContent: "window.open = window.originalOpenFunction" })
                                ...
                                },
                                {
                                    ...
                                    callback: function () {
                                        chrome.extension.sendMessage({ name: "showOptions" });
                                    }
                                },
                                {
                                    label: chrome.i18n.getMessage("donate"),
                                    callback: function () {
                                        window.open('https://paypal.me/ElnurMusayev/5usd');
                                    }
                                }
                            ]
                        })
        ...
        if (!whitelist.includes(domain) && !whitelist.includes(domainWww)) {
        ...
        chrome.runtime.onMessage.addListener(function (msg, sender, response) {
            ...
            "hangouts.google.com", "reddit.com", "googletagmanager.com", "connect.facebook.net", "graph.facebook.com", "widgets.pinterest.com"];
    
            if (msg.from === "popupFeedback") {
                ...
                chrome.runtime.sendMessage({
                    from: 'contentForFeedback',
                    data: { domain: document.domain, url: window.location.href, scripts }
                });
            }
        });
